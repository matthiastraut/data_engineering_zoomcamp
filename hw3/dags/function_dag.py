...
URL_PREFIX = "https://d37ci6vzurychx.cloudfront.net/trip-data/yellow_tripdata_2024-"
...
def upload_to_gcs(month: str, bucket_name: str):
    ...
    file_name = f"yellow_tripdata_2024-{month}.parquet"
    local_path = f"/tmp/{file_name}"
    gcs_path = f"yellow_taxi_2024/{file_name}"
...
def cleanup_file(month: str):
    ...
    local_path = f"/tmp/yellow_tripdata_2024-{month}.parquet"
...
with DAG(
    dag_id="hw3_yellow_taxi_to_gcs",
    default_args=default_args,
    schedule="@once",
    catchup=False,
    description="Loads Jan-Jun 2024 Yellow Taxi Parquet files to GCS for HW3",
    tags=['zoomcamp', 'hw3'],
) as dag:
